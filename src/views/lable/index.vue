<template>
  <div class="app-container">
    <div class="filter-container">
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate(-1)">添加</el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column label="创建时间" width="350px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleUpdate(scope.$index)">编辑</el-button>
        ...
        <el-form-item label="名称" prop="title">
          <el-input v-model='data.name'/>
				...
				<el-form-item label="分类名称">
					<el-select v-model="data.pid" placeholder="请选择分类" @change="change">
						<el-option v-for="item in pid" :label="item.name" :value="item.id" :key="item.id"/>
					</el-select>
				</el-form-item>
				<el-form-item label="分类名称">
					<el-select v-model="data.type" placeholder="请选择分类" @change="change">
  ...
  methods: {
    getList() {
      this.listLoading = true
			let that = this;
			api.assess_label_index(this.page).then((res)=>{
...
//       })
    ...
    handleModifyStatus(id,index) {
          ...
          type: 'warning'
					...
					api.assess_label_remove(data).then((res)=>{
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
					api.assess_label_save(this.data).then((res) => {
