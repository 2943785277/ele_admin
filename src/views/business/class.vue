<template>
  <div class="app-container">
    <div class="filter-container">
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate(-1)">添加分类</el-button>
    </div>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="data" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="商家名称" prop="title">
          <el-input v-model='data.class_name'/>
        </el-form-item>
  ...
  methods: {
    getList() {
      this.listLoading = true
			let that = this;
			api.business_class_index(this.page).then((res)=>{
...
//       })
    ...
    handleModifyStatus(id,index) {
          ...
          type: 'warning'
					...
					api.business_class_remove(data).then((res)=>{
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
					api.business_class_save(this.data).then((res) => {
