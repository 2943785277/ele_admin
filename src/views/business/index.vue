  ...
  <div class="app-container">
		<div class="filter-container">
			<el-row type="flex" class="row-bg" justify="space-between">
				<div><el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate(-1)">添加商家</el-button></div>
				<div><el-input placeholder="请输入用户名称" v-model="page.keywords" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
				<el-button v-waves class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">搜索</el-button></div>
			</el-row>
		</div>
    <el-table
	  ...
	  <el-table-column label="分类名称">
		...
		<el-table-column label="省份">
		  <template slot-scope="scope">
				<span>{{ scope.row.province + scope.row.city + scope.row.area + scope.row.details }}</span>
		  </template>
		</el-table-column>
		...
		<el-table-column label="操作" align="center" width="230" class-name="small-padding fixed-width">
			<template slot-scope="scope">
				<el-button type="primary" size="mini" @click="handleCreate(scope.row.id)">编辑</el-button>
  ...
  methods: {
    getList() {
      this.listLoading = true
			let that = this;
			api.business_index(this.page).then((res)=>{
...
//       })
    ...
    handleModifyStatus(id,index) {
          ...
          type: 'warning'
					...
					api.business_remove(data).then((res)=>{
